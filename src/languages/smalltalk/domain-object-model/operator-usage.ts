...
export class SmalltalkOperatorUsage implements ISmalltalkExpression {
	...
	public evaluate(
		...
		const evaluatedArguments = this.expressionList.value.map(
			(expr: IExpression<ISmalltalkValue>) => expr.evaluate(localEnvironment, globalInfo)
		);
	...
	protected tryGetExpectedNumArgs(globalInfo: IGlobalInfo<ISmalltalkValue>): number | undefined {
